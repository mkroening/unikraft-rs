...
fn main() -> anyhow::Result<()> {
    ...
    let platform = match Platform::detect() {
        ...
        Err(PlatformDetectError::NoPlatform) => {
            println!("cargo:warning=No platform has been selected.");
            return Ok(());
        }
        Err(err) => {
            return Err(err.into());
    ...
    let config = Config::detect(app_dir, arch, platform);
    ...
    impl Platform {
        pub fn detect() -> Result<Self, PlatformDetectError> {
            ...
            match (linuxu, kvm) {
                ...
                (true, false) => Ok(Self::Linuxu),
                (false, true) => Ok(Self::Kvm),
...
mod config {
    ...
    impl Config {
        pub fn detect(app_dir: PathBuf, arch: Arch, platform: Platform) -> Self {
        ...
        pub fn build_unikraft(&self) -> anyhow::Result<()> {
            ...
            for object_file in self.find_object_files()? {
        ...
        pub fn create_static_library(&self) -> io::Result<()> {
            ...
            println!("cargo:rustc-link-lib=static=unikraft");
        ...
        pub fn create_linker_scripts(&self) -> io::Result<()> {
            fs::copy(
                ...
                self.out_dir.join("unikraft_default_linker_script.ld"),
            )?;
